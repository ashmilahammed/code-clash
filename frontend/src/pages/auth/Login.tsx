...
const Login: React.FC = () => {
  ...
  useLayoutEffect(() => {
    const updateWidth = () => {
      // Container max-w-md is 448px. Padding is p-8 (32px each side).
      // Available width is container width - 64px.
      const containerWidth = Math.min(448, window.innerWidth);
      const availableWidth = containerWidth - 64;
      
      // Google GSI button width must be between 200 and 400 pixels.
      const finalWidth = Math.max(200, Math.min(380, availableWidth));
      setGoogleButtonWidth(finalWidth.toString());
    };
