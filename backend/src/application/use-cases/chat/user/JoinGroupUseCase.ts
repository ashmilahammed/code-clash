...
export class JoinGroupUseCase {
    ...
    async execute(conversationId: string, userId: string): Promise<Conversation> {
        ...
        const memberGroupsCount = await this._conversationRepository.countUserGroups(userId);
        if (user) {
            await this._badgeRewardService.checkAndReward(user, Badge.REQUIREMENT_TYPES.GROUP_JOINED, memberGroupsCount);
        }
